# Unified Vulnerability Scanner Platform — Architecture Spec

## Project Overview

Build a modular vulnerability scanning platform that wraps multiple open-source scanning engines behind a unified orchestration layer. Each engine is a **toggleable plugin** that can be enabled/disabled via a configuration UI, updated independently from upstream sources, and produces normalized output consumable by a single reporting pipeline.

The platform is called **VulnForge** (working title — rename as needed).
...
- Translate VulnForge scan profiles into GVM scan configs
- Map GVM task states to VulnForge job states
...
- OR embed as a Go library if VulnForge core is Go-based
...
**Adapter responsibilities:**
- Generate `config.toml` dynamically from VulnForge target definitions
...
**Purpose:** Allow users (Steven and future contributors) to create new plugins without modifying VulnForge core.

**Plugin discovery:**
- Plugins live in a designated directory (e.g., `~/.vulnforge/plugins/`)
- Each plugin is a directory containing a manifest file (`plugin.yaml`) and an executable or script
- VulnForge scans the plugin directory at startup and registers valid plugins
...
engine:
  ...
  args_format: "json"       # how VulnForge passes scan config: json | cli-flags | env
...
All plugins MUST emit results conforming to this schema. The normalizer in VulnForge core validates and enriches.
  ...
  plugins:
    ...
    nuclei:
      ...
      config:
        ...
        custom_templates_dir: "~/.vulnforge/nuclei-custom/"
    ...
    custom_plugins_dir: "~/.vulnforge/plugins/"
```
...
```bash
vulnforge plugin enable nuclei
vulnforge plugin disable openvas
vulnforge plugin list                    # show all plugins and status
vulnforge plugin status nuclei           # detailed health + version info
vulnforge plugin update nuclei           # trigger manual update
vulnforge plugin update --all            # update all enabled plugins
```
...
ScottyScan is not just another plugin -- it is the **enrichment backbone** of VulnForge. External engines (OpenVAS, Nuclei, Nmap, Vuls) detect vulnerabilities against IP:port targets. ScottyScan provides the context that makes those findings actionable.
...
ScottyScan needs the following to serve as a proper VulnForge adapter:
- **Normalized JSONL emitter** -- parallel output matching the VulnForge finding schema
- **Host context block** -- OS, software inventory, domain info as enrichment data that VulnForge can attach to findings from ANY engine
- **Discovery export** -- ScottyScan's discovered host list as input to VulnForge's target definitions, so all engines scan the same environment ScottyScan mapped
...
```
vulnforge workbench                  # launch on default port 8484
vulnforge workbench --port 9090      # custom port
vulnforge workbench --package latest # open directly to most recent scan
```
...
- [ ] Normalized JSONL output (VulnForge finding schema)

### Phase 2: VulnForge Foundation
...
- [ ] ScottyScan adapter (bridge PowerShell scanner into VulnForge plugin interface)
...
- [ ] CLI queries (`vulnforge query --cve CVE-2024-1234 --status open`)
